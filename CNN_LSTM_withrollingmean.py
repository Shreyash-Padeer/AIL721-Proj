...
data["Rolling_Mean"] = data.groupby("Store")["Weekly_Sales"].transform(lambda x: x.rolling(window=4, min_periods=1).mean())

scaler = MinMaxScaler()
scaled_cols = ["Weekly_Sales", "Temperature", "Fuel_Price", "CPI", "Unemployment", "Size", "Rolling_Mean"]
data[scaled_cols] = scaler.fit_transform(data[scaled_cols])

feature_cols = ["Store", "Temperature", "Fuel_Price", "CPI", "Unemployment", "IsHoliday", "Size", "Rolling_Mean"]
...
def train_model(model, train_dl, val_dl, epochs=10, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1} | Train Loss: {np.mean(train_losses):.4f} | Train MSE: {train_mse:.4f} | Val MSE: {val_mse:.4f} | R2: {val_r2:.4f}")


model =  CNNLSTMModel(input_size=len(feature_cols))

print(f"\nTraining CNN_LSTM model")
train_model(model, train_dl, val_dl, epochs=40)
