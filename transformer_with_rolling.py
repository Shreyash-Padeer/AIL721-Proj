...
class TransformerBlock(layers.Layer):
    def __init__(self, embed_dim, num_heads, ff_dim, rate=0.1):
        ...
        self.ffn = tf.keras.Sequential([
            layers.Dense(ff_dim, activation="gelu"),  # GELU often performs better than ReLU
...
embed_dim = 64  # Must match the output_dim in PositionalEmbedding
num_heads = 6   # Number of attention heads
ff_dim = 128    # Hidden layer size in feed forward network

# Build model
inputs = Input(shape=(sequence_length, X_train_seq.shape[2]))
# First project input to embedding dimension
